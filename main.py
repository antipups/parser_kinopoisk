...
def get_year(html_code):
    if html_code.find('"@context"') > -1:
        ...
        return re.search(r'год.*>\d{4}<\/a', html_code, flags=re.DOTALL).group()[-7:-3]
...
def get_director(html_code):
    list_of_directors = []
    if html_code.find('"@context"') > -1:
        for director in eval(re.search(r'director":[^]]*', html_code).group()[10:] + ']'):
            list_of_directors.append(director.get('name'))
    else:
        incorrect_directors = re.search('itemprop="director">((?!<\/tr).)*', html_code).group()[21:]
...
def read_one_film(film_url):
    print('Начинаю загружать - ', film_url[5:-1])
    ...
    print('Фильм - ', film_url[5:-1], ' добавлен в БД')
